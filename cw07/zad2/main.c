...
int main(int argc, const char *argv[])
{
    ...
    sem_t *semBarber = semaphoreCreate(SEM_BARBERS, 0);
    sem_t *semChair = semaphoreCreate(SEM_CHAIRS, 0);
    sem_t *semShmMem = semaphoreCreate(SHARED_MEM, 1);
    sem_t *semQueue = semaphoreCreate(SEM_QUEUE, CHAIRS);
    ...
    semaphoreClose(semBarber);
    semaphoreClose(semQueue);
    semaphoreClose(semChair);
    semaphoreClose(semShmMem);
